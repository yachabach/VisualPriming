...
g <- ggplot(dtPlot)
gg <- g+geom_histogram(aes(x = Xtd, fill = Treatment), bins = 20, alpha = 0.25, size = 2)
gg <- gg+geom_vline(xintercept = mean(dtPlot[Treatment==FALSE,Xtd]), color = "red", size = 2, alpha=.25)
gg <- gg+geom_vline(xintercept = mean(dtPlot[Treatment==TRUE,Xtd]), color = "lightblue", size = 2, alpha=.5)
